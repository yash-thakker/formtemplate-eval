...
export const geminiProAdapter: ExtractionAdapter = {
  name: 'gemini-pro',
  description: 'Google Gemini 2.5 Pro — PDF in, structured template out (no separate OCR).',
  async extract(pdfPath: string, meta: FixtureMeta): Promise<AdapterResult> {
    ...
    return runLLMExtraction(
      { model: google('gemini-2.5-pro'), pricingKey: 'gemini-2.5-pro', pdfPath },
      meta,
    );
  },
};
